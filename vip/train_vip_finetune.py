...
def make_network(cfg):
    print("Loading VIP pretrained model...")
    model = load_vip()  # Loads the default VIP weights

    # for name, param in model.named_parameters():
    #     # print(name)
    #     if not name.startswith('module.convnet.fc'):  # or whatever your final layer name is
...
class Workspace:
    def __init__(self, cfg, logging=True):
        ...
        val_iterable = VIPBuffer(datasource=self.cfg.dataset, datapath=self.cfg.val_datapath, num_workers=self.cfg.num_workers, doaug=0)
    ...
    def setup(self):
        # create logger
        self.logger = Logger(self.work_dir, use_tb=True, cfg=self.cfg)
...
@hydra.main(config_path='cfgs', config_name='config_vip_finetune')
def main(cfg):
    from train_vip_finetune import Workspace as W
    root_dir = Path.cwd()
    workspace = W(cfg)
