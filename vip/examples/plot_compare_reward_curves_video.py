...
def main(rep='vip'):
    ...
    # --- 1) Process success.mp4 ---
    success_path = "strawb/success.mp4"
    success_frames = get_video_frames(success_path)
    success_embeddings, success_frames_tensor = get_embeddings(success_frames, model, transform, rep)

    # The last frame of the success video is our goal
    goal_embedding = success_embeddings[-1]
    goal_frame_tensor = success_frames_tensor[-1]

    # --- 2) Process fail.mp4 using the success goal ---
    fail_path = "strawb/success.mp4"
    fail_frames = get_video_frames(fail_path)
    fail_embeddings, fail_frames_tensor = get_embeddings(fail_frames, model, transform, rep)

    # Distances for the fail video using the success goal embedding
    distances_fail = np.linalg.norm(fail_embeddings - goal_embedding, axis=1)

    # Animate and save fail results
    outpath_fail = f"embedding_curves/succcess_test_{rep}.gif"
    animate_distance(
        distances=distances_fail,
        frames_tensor=fail_frames_tensor,
        goal_frame_tensor=goal_frame_tensor,  # still use success's goal frame for the "Goal Frame" display
